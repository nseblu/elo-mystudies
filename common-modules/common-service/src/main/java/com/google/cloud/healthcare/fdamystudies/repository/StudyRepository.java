...
public interface StudyRepository extends JpaRepository<StudyEntity, String> {
  ...
  @Query(
      value =
          "SELECT created_time AS createdTimestamp, study_id AS studyId, custom_id AS customId, name AS studyName, type AS type, status AS status,"
  ...
  @Query(
      value =
          "SELECT COUNT(prs.id) "
              ...
              + "AND  (prs.email LIKE %:searchTerm% OR loc.name LIKE %:searchTerm% ) ",
      nativeQuery = true)
  public Long countParticipantsByStudyIdAndSearchTermForOpenStudy(
